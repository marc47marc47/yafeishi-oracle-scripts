...
and value = 'enable') or name = 'log_archive_format')order by 1,2,3;

PROMPTPROMPT ASH THRESHOLD...
PROMPT
...
column program format a4 0 tru
...
column minute format a12 tru
column event format a30 tru
column progra! m format a40 wra
...
PROMPT see if you can correlate th! ose with other related wait even ts.
...
PROMPT 11.1: "redo write broadcast ack time" in centiseconds (100 per seco! nd)
...
to_char(dhs.end_int! erval_time,'Mondd_hh24mi') END,< BR>en.name, se.time_waited_micro/1000 total_wait_time, se.total_waits,
...
select v.ve! rsion, ss.snap_id, ss.instance_n umber inst, sn.stat_name, ss.value,
decode(substr(version,1,4),
...
PROMPT
PROMPT APPROACH: These are the AWR snaps where the average 'log file sync'
PROMPT times were the high! est. Look at related waits at th ose times.
...
PROMPT Note: Th! is query won't work on 10.2 - OR A-942
PROMPT
PROMPT APPROACH: Look at the wait distribution for log file sync waits
PROMPT by looking at "wait_time_milli". Look at the high wait times then
...
select to_char(sysdate,'Mo! ndd hh24:mi:ss') TIME from dual; 
