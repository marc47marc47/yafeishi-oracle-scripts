...
PROMPT see if you can correlate those with other related wait even ts.
...
to_char(dhs.end_interval_time,'Mondd_hh24mi') END,< BR>en.name, se.time_waited_micro/1000 total_wait_time, se.total_waits,
...
select v.version, ss.snap_id, ss.instance_n umber inst, sn.stat_name, ss.value,
decode(substr(version,1,4),
...
PROMPT
PROMPT APPROACH: These are the AWR snaps where the average 'log file sync'
PROMPT times were the highest. Look at related waits at th ose times.
...
PROMPT Note: This query won't work on 10.2 - OR A-942
PROMPT
PROMPT APPROACH: Look at the wait distribution for log file sync waits
PROMPT by looking at "wait_time_milli". Look at the high wait times then
