 ...
 */
 ...
 PIPE ROW('Note:438755.1 Version Count Report Version 3.2.3 -- Today''s Date '||to_char(sysdate,'dd-mon-yy hh24:mi')) ;
 PIPE ROW('RDBMS Version :'||v_version||' '||v_instance);

 
    debugme('fetch '||v_sql_id||' '||v_hash);
    fetch vc into v_sql_id, v_query,v_hash,v_addr,v_mem,v_parses,v_execs,v_obs;
    if vc%notfound then
